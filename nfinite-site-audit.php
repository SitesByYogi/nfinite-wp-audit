...
define('NFINITE_AUDIT_VER', '0.4.9');
...
function nfinite_render_caching_layers_page() {
    ...
    if ( ! empty($scan['headers']) ) {
        ...
        $keys = [
            'cf-cache-status','x-cache','x-cache-status','x-nginx-cache','x-nginx-cache-status',
            'x-proxy-cache','x-varnish','age','x-fastcgi-cache','x-srcache-store-status',
            'x-srcache-fetch-status','x-accel-expires','x-litespeed-cache','server'
        ];
        foreach ($keys as $k) {
            $kl = strtolower($k);
